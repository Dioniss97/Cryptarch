...
| **git-pr** | [`.cursor/agents/git-pr.md`](.cursor/agents/git-pr.md) | Ramas `CRYPT-123-slug` (o `docs/<slug>`); commits y **titulo de PR** `CRYPT-123 type(scope): …` (Conventional Commits + prefijo Jira); push y PRs a `develop`. Cuerpo del PR enlaza Jira y Engram `tasks/CRYPT-*`. |
